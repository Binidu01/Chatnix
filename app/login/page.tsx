...
export default function LoginPage() {
  ...
  const [showEmailBraces, setShowEmailBraces] = useState(true);
  const [showPasswordBraces, setShowPasswordBraces] = useState(false);
  ...
  useEffect(() => {
    getRedirectResult(auth)
      ...
      .catch(() => setError("Failed to complete login. Please try again."));
  }, []);
  ...
  const handleEmailKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && email.trim()) {
      e.preventDefault();
      setIsAnimating(true);
      setShowEmailBraces(false);
      setTimeout(() => {
        setShowPasswordBraces(true);
  ...
  const handlePasswordBlur = () => {
    if (password === "" && !isAnimating) {
      setShowPasswordBraces(false);
      setShowEmailBraces(true);
    }
  };
  ...
  return (
    <main className={`login-main ${isLoading ? "login-loading" : ""}`}>
      <div className="login-box">
        <h1 className="login-logo">
          CHAT<span className="login-green">N</span>IX
        </h1>
        ...
        <form onSubmit={handleLogin} className="login-form">
          ...
          <div className="input-group">
            <span
              className={`login-braces ${
                showEmailBraces ? "fade-in" : "fade-out"
              }`}
            >
              {"{"}
            </span>
          ...
          <div className="input-group">
            <span
              className={`login-braces ${
                showPasswordBraces ? "fade-in" : "fade-out"
              }`}
            >
              {"{"}
            </span>
